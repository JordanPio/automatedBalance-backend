...
app.get("/totalPagar", balanceController.apiGetTotalAccPayable);
...
//     }
//     const receberTabela = await pool.query(
//       `select data, SUM(saldo) as TOTAL from apagar where data='${currentBalanceDate}'group by data`
//     );
...
app.get("/pagarTabela", balanceController.getApiAccPayableTable);
...
//     }
//     const receberTabela = await pool.query(
//       `select conta, sum(saldo) as total, (sum(saldo)/sum(sum(saldo)) over()) as percenttotal from apagar where data ='${currentBalanceDate}' group by conta order by total desc`
//     );
//     res.json(receberTabela.rows);
//   } catch (error) {
//     console.error(error.message, "when requesting pagarTabela");
...
app.get("/pagasTabela", balanceController.apiGetAccPaidTable);
...
//   }
// });
app.get("/pagasTest", balanceController.apiGetContasPagas);
...
app.get("/pagasByContas", balanceController.apiGetBillsByAccType);
...
app.get("/pagasByDescricao", balanceController.apiGetBillsByDescription);
...
app.get("/devolucoes", balanceController.apiGetPurchaseReturns);
...
app.get("/cashflowReceber", balanceController.apiGetCashflowReceivable);
...
//     }
//     const receberTabela = await pool.query(
//       `select date_trunc('week', vencimento::date) as weekly, sum(saldo) as receber from receber where descricao not like '%CREDITO%' and data='${endDate}' group by weekly order by weekly`
//     );
//     res.json(receberTabela.rows);
//   } catch (error) {
...
//   }
// });
app.get("/cashflowPagar", balanceController.apiGetCashflowPayable);
...
//     const receberTabela = await pool.query(
//       `select date_trunc('week', vencimento::date) as weekly, sum(saldo) from apagar where data='${endDate}' group by weekly order by weekly`
//     );
//     res.json(receberTabela.rows);
//   } catch (error) {
...
app.get("/totalVendas", balanceController.apiGetTotalSales);
...
//     }
//     const receberTabela = await pool.query(
//       `select sum(totalliquido) as TotalVendas from vendasperiodo where data >='${prevBalanceDate}'and data <='${currentBalanceDate}' order by totalvendas desc`
//     );
//     res.json(receberTabela.rows);
//   } catch (error) {
//     console.error(error.message);
...
app.get("/vendasdre", balanceController.apiGetDreSales);
...
//     }
//     const receberTabela = await pool.query(
//       `select sum(totalvendas) as TotalVendido, sum(custototal) as TotalCusto, sum(lucro) as TotalLucro from vendastotais where de='${prevBalanceDate}' and ate='${currentBalanceDate}'`
//     );
//     res.json(receberTabela.rows);
//   } catch (error) {
//     console.error(error.message);
...
app.get("/vendasOnline", balanceController.apiGetOnlineSales);
...
app.get("/receberTabela", balanceController.apiGetAccReceivable);
...
//     }
//     const receberTabela = await pool.query(
//       `select cliente, sum(saldo) as TOTAL, (sum(saldo)/sum(sum(saldo)) over ()) as percentage from receber where descricao not like '%CREDITO%' and data='${currentBalanceDate}' group by cliente order by total desc`
//     );
//     res.json(receberTabela.rows);
//   } catch (error) {
//     console.error(error.message);
...
app.get("/receberAtrasadas", balanceController.apiGetAccReceivableDue);
...
//     }
//     const receberTabela = await pool.query(
//       `select cliente, sum(saldo) total from receber where descricao not like '%CREDITO%' and data='${currentBalanceDate}' and vencimento < now()::date group by cliente order by total desc`
//     );
//     res.json(receberTabela.rows);
//   } catch (error) {
//     console.error(error.message);
...
app.get("/totalEstoque", balanceController.apiGetStock);
...
//     }
//     const receberTabela = await pool.query(
//       `select date, SUM(custototal) as custototal from estoque where date ='${currentBalanceDate}' group by date`
//     );
//     res.json(receberTabela.rows);
//   } catch (error) {
//     console.error(error.message);
...
//       "select conta, tipo, json_object_agg(data, (total, id) ORDER BY data) FROM ( SELECT conta, tipo, data, total, id FROM balanco GROUP BY conta, tipo, data, total, id) s GROUP BY conta, tipo ORDER BY conta"
//     );
...
//     const ativos = await pool.query(
//       `SELECT * FROM balanco where data='${date}' order by conta`
//     );
...
//       `DELETE FROM balanco where data='${date}'`
//     );
//     res.json("Success");
//   } catch (error) {
//     console.error(error.message);
...
app.post("/insertBalance", balanceController.apiInsert);
...
//       );
//     } else {
//       const cs = new pgp.helpers.ColumnSet(["tipo", "conta", "total", "data"], {
//         table: "balanco",
//       });
