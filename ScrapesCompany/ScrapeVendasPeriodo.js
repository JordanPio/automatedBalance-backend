...
exports.scrape = async (browser, startDate, endDate, errCounter) => {
  ...
  try {
    ...
    // //  Isolated script uncommented until here
    ...
    await page.waitForSelector("td", { timeout: 60000 }); // this is 60sec 60000
    ...
    for (let i = 0; i < rowsData.length; i++) {
      ...
      if (validation === 1) {
        await manipulate.insertVendasPeriodo(rowsData);
      } else {
    ...
    await page.close();
    await console.log("Scrape Vendas Periodo Failed, we are trying again, please wait....", errCounter);
    if (errCounter > 0) {
      await exports.scrape(browser, startDate, endDate, errCounter - 1);
    } else {
      console.log("Application tried to scrape Vendas Periodo more than 3 times and failed");
    }
  }

  // await browser.close(); // use this only if using this script alone to close the browser
  await page.close(); //
};
...
// Run the script
// exports.scrape(startDate, endDate);
