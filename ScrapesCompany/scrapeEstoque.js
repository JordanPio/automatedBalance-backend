...
exports.scrape = async (browser, errCounter) => {
  ...
  try {
    ...
    for (let i = 0; i < rowsData.length; i++) {
      ...
      if (validation === 1) {
        await manipulate.insertEstoqueData(rowsData);
    ...
    await page.close();
    console.log("Scrape Estoque Failed, we are trying again, please wait....", errCounter);
    if (errCounter > 0) {
      await exports.scrape(browser, errCounter - 1);
    } else {
      console.log("Application tried to scrape Estoque more than 3 times and failed");
    }
  }

  // await browser.close();
  // console.log("Estoque Scrapped Succesfully! :)");
  await page.close();
};
