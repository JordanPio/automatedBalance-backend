...
exports.scrape = async (browser, startDate, endDate, errCounter) => {
  ...
  try {
    ...
    await page.close();
    await console.log("Scrape Contas Pagas Failed, we are trying again, please wait....", errCounter);
    if (errCounter > 0) {
      await exports.scrape(browser, startDate, endDate, errCounter - 1);
    } else {
      console.log("Application tried to scrape Contas Pagas more than 3 times and failed");
    }
  }
