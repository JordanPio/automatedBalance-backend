...
exports.scrape = async (browser, errCounter) => {
  ...
  try {
    ...
    for (let i = 0; i < rowsData.length; i++) {
      ...
      if (validation === 1) {
        await manipulate.insertReceber(rowsData);
        // console.log("Scrape was Successful"); // dont need since msg is inside insert now
      } else {
    ...
    await page.close();
    await console.log("Scrape Contas Receber failed, are trying again, please wait....", errCounter);
    if (errCounter > 0) {
      await exports.scrape(browser, errCounter - 1);
    } else {
      console.log("Application tried to scrape Contas a Receber more than 3 times and failed");
    }
  }
